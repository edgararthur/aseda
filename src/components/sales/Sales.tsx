...
export default function Sales() {
	...
	useEffect(() => {
		console.log('Fetching sales data:', page);
		const fetchSales = async () => {
			setLoading(true);
			try {
				const start = (page - 1) * rowsPerPage;
				const end = page * rowsPerPage - 1;
				const { data, error, count } = await supabase.from('sales').select('*', { count: 'exact' }).range(start, end);
							...
							<div className='min-w-full h-full p-3 border-gray-200 border bg-white rounded-md'>
								...
								<table className="min-w-full rounded-lg mt-5">
									<thead className='text-gray-500 text-xs font-normal'>
										<tr>
											<th className='py-2 px-4 border-b text-gray-700 font-medium'>Customer Name</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Reference</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Date</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Sale Status</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Grand Total(GHS)</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Paid</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Due</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Payment Status</th>
