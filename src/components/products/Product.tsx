...
export function Product() {
	...
	async function addSale(saleRecord) {
		try {
			const { data, error } = await supabase.from('products').insert([saleRecord]);
			if (error) throw new Error(error.message);
			fetchProducts();
					...
					<div className="p-6">
						...
						<header className="flex justify-between items-center mb-4">
							<div className="flex justify-between align-middle w-full">
								<div>
									<h1 className="text-lg font-medium text-gray-700">Product List</h1>
						...
						{/* Product Name SKU	Category	Price	Unit	Paid	Tax Rate	Stock Quantity */}
						{transactionOpen && (
							<div className="min-w-72 flex align-middle justify-center ">
								<div className="gap-4 flex flex-col align-middle justify-center bg-white shadow-md p-6 rounded-lg">
								{/* Amount Input */}
									<div className="flex align-middle justify-between">
										<div className='w-full mr-5'>
											...
											name="customer name"
											required
											className="w-full px-4 py-3 border text-gray-800 border-gray-300 rounded-md shadow-sm bg-transparent text-sm focus:outline-none"
										...
										<div className='w-full mr-5'>
											...
											name="customer name"
											required
											className="w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm bg-transparent text-sm focus:outline-none"
									...
									<div className="flex align-middle justify-between gap-4">
										...
										<div className='w-full'>
											...
											<Select options={productPaid} className="w-full text-xs" />
										</div>
									</div>
									...
									<div className="col-span-full flex justify-center">
										...
										className="w-full sm:w-auto px-6 py-3 bg-blue-500 text-white rounded-md flex items-center gap-2 justify-center text-sm font-medium transition"
										>
							...
							<div className='min-w-full h-full p-3 border-gray-200 border bg-white rounded-md'>
									...
									<div className="bg-white p-4 rounded mb-4">
										<div className="flex gap-4 mb-4">
											...
											<div>
												...
												<input type="checkbox" name="100 - 200" id="" placeholder='100 - 200'/>
											</div>
											<div>
								...
								<table className="min-w-full rounded-lg mt-5">
									<thead className='text-gray-500 text-xs font-normal'>
										<tr>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Product Name</th>
											<th className='py-2 px-4 border-b text-gray-700 font-medium'>SKU</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Category</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Price</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Unit</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Paid</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Tax Rate</th>
											<th className="py-2 px-4 border-b text-gray-700 font-medium">Stock Quantity</th>
									...
									<tbody>
										{products.length === 0 ? (
											<tr>
												<td colSpan={9} className="text-center text-xs pt-2 font-medium text-gray-700">No sales data available.</td>
											</tr>
										) : (
											products.map((sale, index) => (
												<tr key={sale.id} className='cursor-pointer text-center text-gray-500 text-xs font-norma'>
													...
													<td className="py-2 px-4 border-b">
														<span className={`px-2 py-1 rounded ${sale.sale_status === 'Paid' ? 'text-green-600' : 'text-orange-400'}`}>
															{sale.sale_status}
														</span>
													</td>
								...
								<footer className="flex justify-between items-center mt-4">
									...
									<div className="flex gap-2 pr-2">
										<button 
											className="px-1 py-1 border border-gray-500 bg-transparent border-none cursor-pointer outline-none text-gray-500 text-xs" 
											onClick={handlePreviousPage} 
											disabled={page === 1}
										>
											<LuChevronsLeft size={15} color='#3a3a3a'/>
										</button>
										<button 
											className="px-1 py-1 bg-transparent border-none outline-none cursor-pointer text-xs" 
											onClick={handleNextPage} 
											disabled={page === totalPages}
										>
											<LuChevronsRight size={15} color='#3a3a3a'/>
										</button>
									</div>
								</footer>
							</div>
